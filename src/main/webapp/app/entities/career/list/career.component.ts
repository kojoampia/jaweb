...
export class CareerComponent implements OnInit {
  ...
  trackId = (_index: number, item: ICareer): string => this.careerService.getCareerIdentifier(item);
  ...
  delete(career: ICareer): void {
    const modalRef = this.modalService.open(CareerDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.careers = this.refineData(dataFromBody);
  }

  protected refineData(data: ICareer[]): ICareer[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: ICareer[] | null): ICareer[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.careerService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
