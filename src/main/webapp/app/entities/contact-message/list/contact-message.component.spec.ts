...
import { sampleWithRequiredData } from '../contact-message.test-samples';
...
describe('ContactMessage Management Component', () => {
  let comp: ContactMessageComponent;
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule, ContactMessageComponent],
      providers: [
        {
      ...
      .overrideTemplate(ContactMessageComponent, '')
      .compileComponents();
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.contactMessages?.[0]).toEqual(expect.objectContaining({ id: 'ABC' }));
  });

  describe('trackId', () => {
    it('Should forward to contactMessageService', () => {
      const entity = { id: 'ABC' };
      jest.spyOn(service, 'getContactMessageIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getContactMessageIdentifier).toHaveBeenCalledWith(entity);
