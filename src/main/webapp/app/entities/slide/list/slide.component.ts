...
import { EntityArrayResponseType, SlideService } from '../service/slide.service';
...
export class SlideComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISlide): string => this.slideService.getSlideIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.slides = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ISlide[] | null): ISlide[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const slidesNew = this.slides ?? [];
      if (data) {
        for (const d of data) {
          if (slidesNew.map(op => op.id).indexOf(d.id) === -1) {
            slidesNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.slideService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
